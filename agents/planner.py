...
- GUI 검증은 Phase마다 앱을 초기화하지 않습니다 -- 로그인 세션이나 저장된
  데이터는 이전 Phase까지의 상태 그대로 이어집니다. 새로 만드는 Phase의
  success_criteria가 "초기 상태"나 "비로그인 상태"를 전제로 한다면, 그
  상태로 되돌아갈 수 있는 기능(로그아웃 등)이 이미 구현되어 있는지
  확인하세요.
- "새로고침/재시작해도 유지"류의 지속성 조건은 하나의 연속된 세션 안에서
  검증할 수 없습니다 (새로고침/재시작 액션이 없음). 별도 Phase로 분리하고,
  그 Phase의 success_criteria는 Phase 시작 직후 첫 화면만으로 확인
  가능하게 작성하세요. 다른 기능을 다 구현한 뒤 맨 마지막에 두면 실패가
  뒤늦게 드러나 그 사이 작업이 낭비되므로 최대한 앞당기되, **로그인/인증
  Phase 바로 다음에는 배치하지 마세요** -- 그 Phase는 로그아웃까지
  테스트하며 끝나므로 그 직후는 항상 로그아웃 상태이고, "이미 로그인
  상태"를 전제로 한 지속성 Phase를 그 자리에 두면 그 어떤 구현으로도
  통과할 수 없는 모순이 됩니다. 로그인 Phase 다음에 (로그아웃 없이 끝나는)
  최소 기능 Phase를 하나 두고, 지속성 Phase는 그 바로 다음에 배치하세요.
"""
...
6. GUI 검증은 각 Phase마다 앱을 초기화하지 않습니다 -- 로그인 세션, 저장된
   계정, 입력한 데이터 등은 이전 Phase까지의 검증에서 남은 상태 그대로
   다음 Phase로 이어집니다 (전체 실행의 맨 처음 한 번만 초기화됩니다).
   그러므로 순서를 정할 때:
   - 로그인/인증 기능이 있는 앱이라면, 로그아웃(또는 다른 초기 상태로
     되돌아가는 방법)은 로그인 기능과 같은 Phase에서 함께 구현하거나
     그보다 먼저 오는 Phase에서 구현하세요. 로그인만 구현하고 로그아웃
     구현을 훨씬 나중 Phase로 미루면, 그 사이의 모든 Phase는 로그인된
     상태에서 벗어날 방법이 없어 "비로그인 상태"를 전제로 한
     success_criteria를 검증할 수 없게 됩니다.
   - 어떤 Phase의 success_criteria가 "초기 상태"나 "비로그인 상태"처럼
     이전 상태로 돌아간 것을 전제로 한다면, 그 시점에 실제로 그 상태로
     되돌아갈 수 있는 기능(로그아웃 버튼 등)이 이미 구현되어 있는지
     반드시 확인하고 순서를 정하세요.
7. "새로고침해도 유지되어야 한다", "다시 시작해도 유지되어야 한다" 같은
   지속성(persistence) 조건은 하나의 연속된 세션 안에서는 검증할 수
   없습니다 -- 화면 검증 시스템에는 새로고침이나 재시작에 해당하는
   액션이 없고 클릭/입력/드래그만 가능합니다. 이런 조건을 데이터를 만드는
   Phase와 같은 Phase에 그대로 넣지 마세요. 대신:
   - 지속성 검증은 별도의 Phase로 분리하고, 요구사항에 있는 다른 기능들을
     (로그인/인증 이외의 모든 핵심 기능은 앱마다 다르므로 구체적으로
     나열하지 않습니다) 전부 구현한 뒤 맨 마지막 Phase로 미루지 말고
     최대한 앞당기세요. 지속성 버그는 재작성(rewrite) 과정에서 상태 파일
     이름/구조가 바뀌는 등으로 흔히 생기는데, 맨 마지막에 배치하면 그
     실패가 다른 모든 기능을 다 구현하고 검증한 뒤에야 드러나 앞선
     작업이 전부 낭비됩니다.
   - **단, 로그인/인증 Phase 바로 다음에 배치하면 안 됩니다.** 위 6번
     규칙에 따라 로그인/인증 Phase는 반드시 로그아웃까지 테스트하며
     끝나므로(그래야 로그아웃 기능 자체를 검증할 수 있음), 그 Phase가
     끝난 시점의 실제 화면 상태는 항상 "로그아웃된 상태"입니다. 그
     직후에 "이미 로그인 상태이다"를 전제로 하는 지속성 Phase를 넣으면,
     그 어떤 구현으로도 만족시킬 수 없는 모순된 계획이 됩니다(로그아웃을
     막 테스트한 직후인데 이미 로그인되어 있어야 한다는 모순).
     대신: 로그인/인증 Phase 다음에 **먼저 다시 로그인하고, 요구사항의
     핵심 기능 중 아무거나 하나를 최소한으로 구현·검증하되 그 Phase
     자체는 로그아웃으로 끝나지 않는 Phase**를 하나 두고, 지속성 Phase는
     그 바로 다음에 배치하세요 (구조 예시 -- 실제 Phase 이름과 개수는
     요구사항에 맞게 정하세요: "로그인/회원가입/로그아웃 구현" -> "로그인
     후 기본 화면과 핵심 데이터 생성 기능 구현"(로그아웃 없이 끝남) ->
     "로그인 상태와 생성한 데이터가 앱을 다시 시작해도 유지된다" -> 이후
     나머지 핵심 기능들). 지속성 Phase의 success_criteria를 작성하기
     전에 반드시 "바로 앞 Phase의 마지막 액션이 실제로 로그인 상태로
     끝나는지"를 확인하세요.
   - 그 Phase의 success_criteria는 "이 Phase 화면을 처음 열었을 때
     이미 로그인 상태이고, 이전에 만든 데이터가 그대로 보인다"처럼, Phase가
     시작되자마자(아무 액션도 하기 전에) 화면에 보이는 것만으로 확인
     가능하게 작성하세요. Phase마다 앱을 초기화하지 않으므로(위 6번
     참고), 새 Phase의 첫 화면 자체가 이전 Phase까지 쌓인 상태를 그대로
     보여주는 것이 곧 "재시작 후에도 유지됨"의 증거입니다.
"""
