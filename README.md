...
> **현재 단계**: 오케스트레이션 뼈대(루프 제어, 상태 저장, 에이전트 인터페이스)만
> 구현되어 있습니다. 각 에이전트의 실제 LLM 로직은 `agents/*.py`의 `Stub*Agent`
> 클래스에 `NotImplementedError`로 남겨두었고, 다음 단계에서 하나씩 채웁니다.
...
│   ├── planner.py           # StubPlannerAgent
│   ├── developer.py         # StubDeveloperAgent
...
│   ├── logging_setup.py       # 콘솔 + /logs 파일 로깅 설정
│   └── pipeline.py            # Orchestrator: 기획→개발↔리뷰↔GUI검증→재계획→에스컬레이션 루프
├── target-app/               # 에이전트들이 생성할 Todo 앱 결과물이 저장되는 위치
├── state/                    # 파이프라인 진행 상태 JSON (pipeline_state.json)
├── logs/                     # 실행 로그
├── main.py                   # 엔트리 포인트 (현재 에이전트가 스텁이라 실행하면 NotImplementedError 발생)
...
> 현재는 각 에이전트가 스텁 상태이므로 `main.py` 실행 시 Planner 단계에서
> `NotImplementedError`가 발생하는 것이 정상입니다. 오케스트레이션 배선이
> 올바른지 확인하는 용도입니다.

## 다음 단계

1. `agents/planner.py`: 요구사항 분할 + 성공 조건 정의 로직 구현
2. `agents/developer.py`: 코드 생성 + 내장 Lint/AST 자체 수정 로직 구현
