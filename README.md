...
Key `.env` knobs: `PLANNER_MODEL` / `DEVELOPER_MODEL` / `REVIEWER_MODEL` /
...
│   ├── planner.py             # create_plan / replan / request_human_escalation / incremental report
...
│       ├── judgment.py                    # Vision-model action decision (image + criteria in, action out)
...
├── state/plan.json          # Single source of truth for every Phase's status
...
`DEBUG_INJECT_PHASE_ID`, skips the Reviewer for one named Phase and sabotages
the Developer's `app.js` before GUI verification, so the GUI Tester catches a
real (if staged) bug and the Developer→GUI Tester rewrite loop runs for real.
...
주요 `.env` 설정: `PLANNER_MODEL` / `DEVELOPER_MODEL` / `REVIEWER_MODEL` /
`GUI_TESTER_MODEL`(에이전트별 모델 지정), `MAX_DEV_REVIEW_RETRIES`,
`MAX_GUI_TEST_RETRIES`, `MAX_REPLAN_ATTEMPTS`, `DEBUG_INJECT_BUG` /
`DEBUG_INJECT_PHASE_ID`(아래 참고).
...
│   ├── planner.py             # create_plan / replan / request_human_escalation / 누적 리포트
...
│       ├── judgment.py                    # Vision 모델 행동 판단 (이미지+성공조건 입력, 행동 출력)
...
├── state/plan.json          # 모든 Phase 상태의 단일 소스
...
켜면, 지정한 Phase의 Reviewer를 건너뛰고 GUI 검증 직전에 Developer의
`app.js`를 일부러 훼손합니다. 그러면 GUI Tester가 (연출된) 진짜 버그를
잡아내고 Developer↔GUI Tester 재작성 루프가 실제로 돕니다. **재계획**까지
